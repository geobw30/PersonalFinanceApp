...
export default function Finance() {
  ...
  const [editInvestmentDialogOpen, setEditInvestmentDialogOpen] =
    useState(false);
  ...
  const [deleteSavingConfirmDialogOpen, setDeleteSavingConfirmDialogOpen] =
    useState(false);
  ...
  const [deleteIncomeConfirmDialogOpen, setDeleteIncomeConfirmDialogOpen] =
    useState(false);
  ...
  const fetchSummaryData = useCallback(async () => {
    try {
      const now = new Date();
      const [savingsRes, summaryRes, expensesRes] = await Promise.all([
        getSavings(),
        getFinancialSummary(),
  ...
  useEffect(() => {
    if (tabValue === 0 && !investmentsLoaded && !loadingInvestments) {
      fetchInvestments();
    }

    if (tabValue === 2 && !incomesLoaded && !loadingIncomes) {
      fetchIncomes();
    }
  }, [
    tabValue,
  ...
  const handleAddInvestment = async (investment: Omit<Investment, "id">) => {
    try {
      ...
      await Promise.all([fetchInvestments(), fetchSummaryData()]);
  ...
  const handleAddIncome = async (income: Omit<Income, "id">) => {
    try {
      ...
      await Promise.all([fetchIncomes(), fetchSummaryData()]);
  ...
  const getAdjustedMonthlyIncome = () => {
    ...
    const totalSavingsThisMonth = savingsThisMonth.reduce(
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
      ...
      <Paper
        elevation={0}
        sx={{ p: 3, borderRadius: 0, borderBottom: 1, borderColor: "divider" }}
      >

...
import { getCategories, getSubCategoriesByCategory, updateExpense } from '../api/client';
...
export default function EditExpenseScreen() {
  ...
  const [subCategoryId, setSubCategoryId] = useState(expense.subCategoryId ? String(expense.subCategoryId) : '');
  ...
  useEffect(() => {
    ...
    void getSubCategoriesByCategory(Number(categoryId)).then((res) => setSubCategories(res.data));
  }, [categoryId]);

  const canSave = useMemo(() => categoryId && description.trim() && Number(amount) > 0, [categoryId, description, amount]);
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.pickerWrap}>
          <Picker selectedValue={subCategoryId} onValueChange={(value) => setSubCategoryId(String(value))}>
            <Picker.Item label='Select sub category' value='' />
            {subCategories.map((subCategory) => (
              <Picker.Item key={subCategory.id} label={subCategory.name} value={String(subCategory.id)} />
            ))}
          </Picker>
        </View>
        ...
        <Button title='Update Expense' onPress={onSave} />
      </ScrollView>
